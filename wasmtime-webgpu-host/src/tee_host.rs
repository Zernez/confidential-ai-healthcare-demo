...
impl TeeHost {
    ...
    fn detect_tee_type_internal(&self) -> TeeType {
        ...
        if std::path::Path::new("/sys/firmware/sev").exists() {
            // Check dmesg for SEV status (already checked in user's output)
    ...
    pub fn attest_vm_sync(&self) -> AttestationResult {
        ...
        match tee_type {
            ...
            TeeType::AmdSev => {
                // AMD SEV without SNP - memory is encrypted but no remote attestation possible
    ...
    fn attest_amd_sev_snp_sync(&self) -> Result<AttestationResult> {
        ...
        // This can be verified offline or by a remote verifier
        ...
        T: AsTeeHost + Send + 'static,
    {
        ...
        // NEW: wasmtime_attestation_detect_tee() -> pointer to JSON string with TEE type
        linker.func_wrap(
            "wasmtime_attestation",
            "detect_tee",
            |mut caller: Caller<'_, T>| -> i32 {
...
// Re-export base64 crate functions
mod base64_utils {
