...
```
┌─────────────────────────────────────────────┐
│  WASM Guest (C++ ML Code)                  │
│  ├─ Calls wasi:webgpu functions             │
│  ├─ create-instance()                       │
│  ├─ request-adapter()                       │
│  ├─ create-buffer()                         │
│  └─ queue-write-buffer()                    │
...
│  ├─ Implements wasi:webgpu imports          │
│  ├─ Maps calls to wgpu API                  │
│  └─ Manages GPU resources                   │
└─────────────────────────────────────────────┘
           ↓
┌─────────────────────────────────────────────┐
│  wgpu Library                               │
│  └─ CUDA / Vulkan / Metal / DX12           │
└─────────────────────────────────────────────┘
           ↓
┌─────────────────────────────────────────────┐
...
│
├── wasmwebgpu-ml/
│   ├── wasi-gfx/                    # Cloned from GitHub
│   ├── wit-bindings/                # Generated bindings
│   │   ├── c/                       # C bindings from wit-bindgen
│   │   ├── cpp/                     # C++ wrapper
│   │   └── wasi_webgpu_cpp.hpp      # C++ interface
│   ├── src/
│   │   ├── gpu_executor.cpp         # Old placeholder
│   │   └── gpu_executor_wit.cpp     # NEW: Real WIT bindings
│   └── shaders/
│       └── *.wgsl                   # WGSL compute shaders
│
├── wasmtime-webgpu-host/           # Custom wasmtime
│   ├── src/
│   │   ├── main.rs                  # Entry point
│   │   ├── webgpu_host.rs           # wasi:webgpu implementation
│   │   └── gpu_backend.rs           # wgpu integration
│   └── Cargo.toml
│
└── Scripts:
    ├── setup_wasi_gfx.sh            # Setup & generate bindings
...
**Status**: 🚧 Beta - GPU initialization works, full compute pipeline in progress  
